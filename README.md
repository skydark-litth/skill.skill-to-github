...
- **上传前泄密 / 隐私检查**：每次上传 / 更新前对技能做文件清单、密钥模式、绝对个人路径、嵌套 `.git` 四项检查；发现任何可疑即停止并汇报，由你决定后再继续。
- **README 保持最新**：缺失时经你同意由大模型自动生成；若远程 README 的版本比技能版本旧，经你同意由大模型按「旧 README + 新技能的变化」更新——保留旧结构语气、刷新版本号 / 特性 / 工作流。
...
- **自动清理克隆目录**：推送验证成功后自动删除本次克隆的工作目录以节约磁盘——只删该临时 `<workdir>`、不触碰原始 skill 目录；推送失败则保留以便重试。
...
| 检查 README | Step 2 | 缺失则问是否生成；远程 README 版本旧于技能则问是否按变化更新 |
| 泄密 / 隐私检查 | Step 3 | 文件清单、密钥模式、绝对路径、嵌套 `.git`；有疑即停并汇报 |
| 覆盖内容 | Step 4 | `cp -r "<skill-dir>/." "<workdir>/"`（保留仓库独有文件） |
...
- 每次上传 / 更新前执行泄密与隐私检查，任何可疑项都会先汇报给你，由你决定后再继续，避免泄露。
- 远程的创建、目标的选择、README 的生成与更新均由用户显式确认，避免误覆盖。

## 版本

- 当前版本：**v2.3.0**（详见 [`SKILL.md`](./SKILL.md) 顶部 `version` 字段）。
